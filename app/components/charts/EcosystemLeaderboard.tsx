    ...
    >
      {!data ? (
        ...
        <div>
          ...
          {/* Header row */}
          <div className="mb-1 grid grid-cols-[28px_1fr_200px_140px_160px] items-center gap-4 px-3 font-mono text-[9px] uppercase tracking-widest text-[var(--color-muted)]">
            <span>#</span>
            <span>Venue</span>
            <span>{oiLabel}</span>
            <span>{volLabel}</span>
          ...
          <div className="flex flex-col gap-0.5">
            {sorted.map((v, i) => {
              ...
              return (
                <div
                  key={v.venue}
                  className="group grid grid-cols-[28px_1fr_200px_140px_160px] items-center gap-4 rounded-lg border border-transparent px-3 py-3 transition-colors duration-150 hover:border-[var(--color-line)] hover:bg-[var(--color-surface)]/40"
                >
                  {/* Rank */}
                  <span className="font-mono text-sm font-semibold" style={{ color: i === 0 ? "#FBBF24" : i === 1 ? "#94A3B8" : "#64748B" }}>
                    {i + 1}
                  </span>
